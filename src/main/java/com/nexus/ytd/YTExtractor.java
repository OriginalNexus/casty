...
public class YTExtractor {
	...
	public String getTitle() throws IOException {
		if (title == null) {
			...
			if (found)
				title = Utils.unescapeJavaScript(m.group("title"));
			else
				title = "";
		}
		return title;
	...
	public String getAuthor() throws IOException {
		if (author == null) {
			...
			if (m.find()) {
				...
				if (m.find())
					author = URLDecoder.decode(m.group("author"), StandardCharsets.UTF_8);
			}
		}
		return author;
	}
	...
	private String getVideoPage() throws IOException {
		if (video_page == null) {
 			video_page = Utils.downloadURLToString(ytUrl.getUrl());
		}
		return video_page;
	}
	...
	private String getVideoInfoPage() throws IOException {
		if (video_info_page == null) {
			for (String el : el_array) {
				...
				if (video_info_page.contains("adaptive_fmts") || video_info_page.contains("url_encoded_fmt_stream_map"))
					break;
			}
		}
	...
	private String getEmbedPage() throws IOException {
		if (embed_page == null) {
			embed_page = Utils.downloadURLToString(ytUrl.getProtocol() + "//www.youtube.com/embed/" + ytUrl.getId());
		}
		return embed_page;
	}
		...
		if (player_url == null) {
			...
			if (found) {
				...
				if (!player_url.startsWith("http"))
					check = new URL(ytUrl.getProtocol() + "//" + player_url);
				else
					check = new URL(player_url);
				if (check.getHost() == null || check.getHost().isEmpty())
					player_url = "www.youtube.com" + player_url;
