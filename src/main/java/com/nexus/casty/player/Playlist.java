...
public class Playlist {
	...
	public synchronized void addItem(PlaylistItem item) {
		playlist.add(item);
		if (index < 0 && (nextIndex < 0 || nextIndex >= playlist.size()))
			nextIndex = playlist.size() - 1;

		CastyServer.getInstance().updateStatus(new PlaylistStatus().setItems(getItems()));
	}
	...
	synchronized PlaylistItem next() {
		...
		this.index = index;
		nextIndex = -1;
		CastyServer.getInstance().updateStatus(new PlaylistStatus().setIndex(index));
	...
	public synchronized void removeItem(int index) {
		...
		CastyServer.getInstance().updateStatus(new PlaylistStatus().setIndex(this.index).setItems(getItems()));
	}
	...
	public synchronized void setRepeat(boolean repeat) {
		this.repeat = repeat;
		CastyServer.getInstance().updateStatus(new PlaylistStatus().setRepeat(repeat));
	}
	...
	synchronized void load(List<PlaylistItem> array) {
		...
		CastyServer.getInstance().updateStatus(new PlaylistStatus().setIndex(index).setItems(getItems()));
	}
	...
	private synchronized int getPreviousIndex() {
		if (playlist.size() == 0) return -1;
	...
	private PlaylistStatus getStatus() {
